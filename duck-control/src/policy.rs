...
pub enum PolicyError {
    ...
    #[error("native policy {path}: {detail}")]
    Native { path: PathBuf, detail: String },
...
impl PolicyError {
    ...
    pub fn path(&self) -> Option<&Path> {
        match self {
            #[cfg(feature = "imx6ull-mlp")]
            PolicyError::Native { path, .. } => Some(path),
...
impl Policy {
    ...
    pub fn load(paths: &PolicyPaths, standing_threshold: f64) -> Result<Self, PolicyError> {
        if std::iter::once(&paths.walk)
            .chain(paths.stand.iter())
            .chain(paths.sitstand.iter())
            .chain(paths.ground_pick.iter())
            .chain(paths.skills.iter())
            .any(|path| !is_native(path))
        {
            ensure_runtime()?;
        }
...
struct Network {
    ...
    #[cfg(feature = "imx6ull-mlp")]
    native: Option<crate::native_mlp::NativeMlp>,
...
impl Network {
    ...
    fn run(&mut self, observation: &Observation) -> Result<[f32; ACTION_LEN], PolicyError> {
        ...
        if let Some(native) = &self.native {
            return native.infer(observation.as_slice()).map_err(fail);
        }
        ...
        let session = self.session.as_mut().expect("an ONNX or native network");
        let outputs = match &self.state {
            Some(state) => session.run(ort::inputs![
                "obs" => &input, "h_in" => &state.h, "c_in" => &state.c
            ]),
            None => session.run(ort::inputs!["obs" => &input]),
        }
...
fn is_native(path: &Path) -> bool {
    cfg!(feature = "imx6ull-mlp") && path.extension().is_some_and(|ext| ext == "duckmlp")
}
...
fn open(path: &Path) -> Result<Network, PolicyError> {
    ...
    #[cfg(feature = "imx6ull-mlp")]
    if is_native(path) {
        let native = crate::native_mlp::NativeMlp::from_bytes(&bytes).map_err(|detail| {
            PolicyError::Native {
                path: path.to_owned(),
                detail,
            }
