...
def run_gui(path):
    ...
    forward = tk.DoubleVar(value=0.10)
    backward = tk.DoubleVar(value=0.08)
    turn = tk.DoubleVar(value=0.40)
    ...
    main = tk.Frame(root, bg="#eff4f2", padx=26, pady=20)
    ...
    text(main, variable=focus_text, size=12, color="#648078", anchor="w").pack(fill="x", pady=(4, 12))

    pad = tk.Frame(main, bg="white", padx=20, pady=15)
    pad.pack(fill="x")
    ...
    sliders.pack(fill="x", pady=(14, 10))
    for title, variable, upper, unit in [("前进速度", forward, 0.30, "m/s"),
                                         ("后退速度", backward, 0.20, "m/s"),
                                         ("转向速度", turn, 1.0, "rad/s")]:
        ...
        def changed(value, label=value_label, suffix=unit):
            label.config(text=f"{float(value):.2f} {suffix}")
        slider = ttk.Scale(row, from_=0.0, to=upper, variable=variable, command=changed)
        slider.pack(side="left", fill="x", expand=True, padx=15)
        def step(event, variable=variable, upper=upper, changed=changed):
